...
class DefaultHigherOrderValidationService : HigherOrderValidationService {
    ...
    override fun validate(input: GreenCertificate, verificationResult: VerificationResult): GreenCertificate {
        ...
        if (numberNonNullVaccinations > 0) {
            if (numberNonNullTests > 0)
                throw VerificationException(Error.SCHEMA_VALIDATION_FAILED, "Vaccination and test entry found")
        ...
        if (numberNonNullRecoveryStatements > 0) {
            if (numberNonNullTests > 0)
                throw VerificationException(Error.SCHEMA_VALIDATION_FAILED, "Recovery and test entry found")
            if (numberNonNullVaccinations > 0)
                throw VerificationException(Error.SCHEMA_VALIDATION_FAILED, "Recovery and vaccination entry found")
        ...
        if (numberNonNullTests == 0 && numberNonNullVaccinations == 0 && numberNonNullRecoveryStatements == 0) {
            throw VerificationException(Error.SCHEMA_VALIDATION_FAILED, "No test, vaccination, or recovery entry")
        }
        return input
    }
}
